            ...
            <div className="container mx-auto px-0 relative z-10">
                    ...
                    <div className="bg-gradient-to-br from-primary/20 to-secondary/20 backdrop-blur-xl  border border-white/40 rounded-[2rem] p-8  max-md:p-16 max-sm:p-10 lg:p-10 shadow-xl shadow-primary/5 relative">
                        ...
                        <div className="flex items-start justify-between  mb-6">
                            <div className=" max-w-[80%]">
                                ...
                                {product.saleMode && config?.rubro?.productFormConfig?.showSaleMode && (
                                    <Badge className={`mt-3 border-none rounded-lg font-semibold shadow-sm ${product.saleMode === 'VENTA' ? 'bg-blue-600/90 text-white' : product.saleMode === 'ALQUILER' ? 'bg-amber-500/90 text-white' : 'bg-emerald-600/90 text-white'}`}>
                                        {product.saleMode === 'AMBOS' ? 'Venta/Alquiler' : product.saleMode}
                                    </Badge>
                                )}
                            </div>
                            <Button
                                    ...
                                    {product.skus.map((sku: SKU) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                disabled={skuStock === 0}
                                                className={`px-4 py-2 rounded-xl text-sm font-medium border-2 transition-all duration-200 relative overflow-hidden ${selectedSku === sku.id
                        ...
                        {/* Controles de compra y Cantidad */}
                        {(config?.rubro?.cartEnabled ?? true) && (
                            <>
                                <div className="mb-8">
                                    <Label className="mb-3 block font-bold text-foreground/90">
                                        {product.allowFractional
                            ...
                            <div className="mb-8 flex justify-center w-full">
                                <a
                                    href={`https://wa.me/${config.contactPhone.replace(/\D/g, '')}?text=${encodeURIComponent(
                                        config?.whatsappProductMessage?.replace('[PRODUCTO]', product.name) ||
                                        `Hola, quiero consultar sobre el producto ${product.name}`
                                    )}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center justify-center gap-2 w-full max-sm:w-60 h-14 rounded-full bg-[#25D366] hover:bg-[#128C7E] text-white font-bold transition-colors shadow-lg shadow-[#25D366]/20 hover:-translate-y-0.5"
                                >
                ...
                {product?.youtubeVideo && (
                    <div className="mb-16 px-6 max-w-4xl mx-auto">
                        <h3 className="text-2xl font-bold mb-6 bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Video de {config?.rubro?.productFormConfig?.productLabel?.toLowerCase() || "producto"}</h3>
                ...
                <div className="mb-20 px-6">
                    <Tabs defaultValue="description" className="w-full">
                        <div className="flex justify-center mb-8">
                            <TabsList className="bg-gray-500/10 backdrop-blur-md border border-zinc-500/40 p-1 rounded-full max-md:rounded-2xl  shadow-lg shadow-black/5 inline-flex h-auto  max-sm:grid max-sm:grid-cols-2 max-md:grid max-md:grid-cols-3 lg:flex-wrap gap-3 max-xl:w-[70%] max-lg:w-[95%] max-md:w-[100%]">
                                {[
                                    "description",
                                    "specifications",
                        ...
                        <div className="bg-secondary/10 backdrop-blur-md border-4 border-zinc-500/20 rounded-[2rem] p-8 lg:p-12 shadow-xl shadow-primary/5 min-h-[300px]">
                            <TabsContent value="description" className="mt-0">
                                <div className="prose prose-lg max-w-none prose-headings:font-bold prose-headings:text-foreground prose-p:text-muted-foreground">
                                    <h3 className="text-2xl mb-4">Sobre {config?.rubro?.productFormConfig?.productLabel ? `este/a ${config.rubro.productFormConfig.productLabel.toLowerCase()}` : "este producto"}</h3>
                                    ...
                                    {reviews.length === 0 ? (
                                        <div className="text-center py-16 bg-white/30 rounded-2xl border border-dashed border-black/10">
                                            ...
                                            <p className="text-xl font-medium text-foreground mb-2">
                                                Aún no hay reseñas para {config?.rubro?.productFormConfig?.productLabel ? `este/a ${config.rubro.productFormConfig.productLabel.toLowerCase()}` : "este producto"}
                                            </p>
                            ...
                            <div className="flex justify-between items-end mb-8">
                                <div>
                                    <h2 className="text-3xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent mb-2">
                                        {config?.rubro?.productFormConfig?.productLabel ? `${config.rubro.productFormConfig.productLabel} Relacionados/as` : "Productos Relacionados"}
                                    </h2>
