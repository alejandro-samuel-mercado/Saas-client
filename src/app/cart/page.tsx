...
function CartContent() {
    ...
    useEffect(() => {
        configService.getPublicConfig().then((config) => {
            if (config?.rubro?.cartEnabled === false) {
                router.push("/");
                toast.error("El carrito no está habilitado para este tipo de tienda.");
            }
        }).catch(console.error);
    }, [router]);
    ...
    useEffect(() => {
        const fetchPaymentOptions = async () => {
            ...
            try {
                ...
                if (storeConfig?.planInfo?.enabledModules?.includes('payment_method_whatsapp')) {
                    manualMethods.push({
                        id: 9998,
    ...
    const canProceed = () => {
        switch (currentStep) {
            ...
            case "delivery":
                ...
                if (storeConfig && storeConfig.enableShipping === false) {
                    return false;
                }
                return (
                    !!customerData.address &&
    ...
    useEffect(() => {
        if (isUpdating) {
            const dbg = setTimeout(() => {
                fetch(`/api/config?debug=${isDebouncing ? '1' : '0'}${previewMutation.isPending ? '1' : '0'}${shippingMutation.isPending ? '1' : '0'}${couponMutation.isPending ? '1' : '0'}${createOrderMutation.isPending ? '1' : '0'}`);
            }, 1000);
            return () => clearTimeout(dbg);
        }
    }, [isUpdating, isDebouncing, previewMutation.isPending, shippingMutation.isPending, couponMutation.isPending, createOrderMutation.isPending]);
            ...
            <div className="container mx-auto px-4 max-w-6xl md:p-20">
                ...
                <div className="mb-12">
                    <div className="flex items-center justify-between max-w-3xl mx-auto relative px-4">
                        ...
                        <div
                            className="absolute top-5 left-4 h-1 bg-gradient-to-r from-primary to-secondary rounded-full -z-10 transition-all duration-500"
                            style={{
                                width: `${(cartContent.stepper.steps.findIndex((s) => s.key === currentStep) / (cartContent.stepper.steps.length - 1)) * 100}%`,
                            }}
                        ></div>

                        {cartContent.stepper.steps.map((step, idx) => {
                            const isActive = currentStep === step.key;
                            const isCompleted =
                                cartContent.stepper.steps.findIndex(
                                    (s) => s.key === currentStep,
                                ) > idx;
                            ...
                            <div className="space-y-3">
                                ...
                                {!user && currentStep === "cart" && (
                                    <div className="mt-4 text-center">
                                        <p className="text-sm text-muted-foreground mb-2">¿Ya tienes una cuenta?</p>
                                        <Button
                                            variant="outline"
                                            className="w-full rounded-full border-primary/40 text-primary hover:bg-primary/5 font-semibold"
