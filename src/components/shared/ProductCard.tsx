...
export const ProductCard = memo(function ProductCard({ product }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    if (isOutOfStock) {
      toast.error(`${config?.rubro?.productFormConfig?.productLabel || "Producto"} agotado/a`);
      return;
    }

    if (!product.skus || product.skus.length === 0) {
      toast.error(`${config?.rubro?.productFormConfig?.productLabel || "Producto"} no disponible`);
      return;
    }
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    if (isOutOfStock) {
      e.preventDefault();
      toast.info(`Este/a ${config?.rubro?.productFormConfig?.productLabel?.toLowerCase() || "producto"} se encuentra agotado/a por el momento.`);
    }
  };
      ...
      >
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-2">
          ...
          {product.saleMode && config?.rubro?.productFormConfig?.showSaleMode && (
            <Badge className={`border-none rounded-lg font-semibold shadow-sm ${product.saleMode === 'VENTA' ? 'bg-blue-600/90 text-white' : product.saleMode === 'ALQUILER' ? 'bg-amber-500/90 text-white' : 'bg-emerald-600/90 text-white'}`}>
