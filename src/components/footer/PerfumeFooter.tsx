...
export function PerfumeFooter() {
    ...
    return (
        <footer className="bg-[#0d0b08] text-[#f9f1d8] border-t border-[#d4af37]/10">
            ...
            <div className="bg-[#d4af37]">
                <div className="container mx-auto px-6 lg:px-12 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <form className="flex w-full md:w-auto gap-0" onSubmit={(e) => e.preventDefault()}>
                        ...
                        />
                        <button type="submit" className="bg-black text-[#d4af37] px-6 py-4 font-bold text-xs tracking-widest uppercase hover:bg-[#1a1614] transition-colors flex items-center gap-2">
            ...
            <div className="container mx-auto px-6 lg:px-12 py-20">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                    ...
                    <div className="lg:col-span-1">
                        <div className="mb-8">
                            {config?.logoUrl ? (
                                ...
                                <div className="mb-4">
                                    <span className="text-[#d4af37] font-serif text-2xl tracking-[0.3em] uppercase block">{config?.storeName || "MAISON"}</span>
                                    <span className="text-white/20 text-[8px] tracking-[0.5em] uppercase">Parfum</span>
                                </div>
                            )}
                                ...
                                {socialLinks.map((social) => {
                                    ...
                                    return (
                                        <Link key={social.platform} href={social.href || "#"} target="_blank"
                                            className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/40 hover:border-[#d4af37]/50 hover:text-[#d4af37] transition-all">
                    ...
                    {/* Colección */}
                    <div>
                        <h4 className="text-[10px] font-bold tracking-[0.4em] uppercase text-[#d4af37] mb-8">Colección</h4>
                        <ul className="space-y-4">
                            {NAV_LINKS.map((l) => (
                                <li key={l.label}>
                                    <Link href={l.href} className="text-white/50 hover:text-[#d4af37] text-sm tracking-wide transition-colors flex items-center gap-2 group">
                    ...
                    {/* Información */}
                    <div>
                        <h4 className="text-[10px] font-bold tracking-[0.4em] uppercase text-[#d4af37] mb-8">Información</h4>
                        <ul className="space-y-4">
                            {INFO_LINKS.map((l) => (
                                <li key={l.label}>
                                    <Link href={l.href} className="text-white/50 hover:text-[#d4af37] text-sm tracking-wide transition-colors flex items-center gap-2 group">
                    ...
                    {/* Contacto */}
                    <div>
                        <h4 className="text-[10px] font-bold tracking-[0.4em] uppercase text-[#d4af37] mb-8">Contacto</h4>
                        <ul className="space-y-5">
                            {branch?.address && (
                                <li className="flex items-start gap-3">
                                    <MapPin className="h-4 w-4 text-[#d4af37] mt-0.5 flex-shrink-0" />
                                    <span className="text-white/50 text-sm leading-relaxed">{branch.address}{branch.city ? `, ${branch.city}` : ""}</span>
                                </li>
                            )}
                            ...
                            {config?.contactEmail && (
                                <li className="flex items-center gap-3">
                                    ...
                                    <a href={`mailto:${config.contactEmail}`}
                                        className="text-white/50 hover:text-[#d4af37] text-sm transition-colors truncate">
                                        {config.contactEmail}
                                    </a>
                                </li>
                            )}
                    ...
                    <div className="flex gap-6">
                        {[
                            ...
                            <Link key={l.label} href={l.href}
                                className="text-white/20 hover:text-[#d4af37] text-[10px] tracking-widest uppercase transition-colors">
                                {l.label}
                            </Link>
                        ))}
                    </div>
