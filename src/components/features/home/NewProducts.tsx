            ...
            <section className="py-16 relative">
                <div className="container mx-auto px-4">
                    <div className="text-center mb-12">
                        <h2 className="text-3xl font-bold mb-2">{rubroConfig.newProductsTitle}</h2>
                        <p className="text-muted-foreground">{rubroConfig.newProductsSubtitle}</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                        {Array.from({ length: 5 }).map((_, idx) => (
                            <div key={idx} className="h-80 bg-muted animate-pulse rounded-xl" />
                        ))}
                    </div>
                </div>
            </section>
                    ...
                    <div>
                        <div className="flex items-center gap-2.5 mb-1">
                            ...
                            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100">
                                {rubroConfig.newProductsTitle}
                            </h2>
                        </div>
                        <p className="text-sm text-gray-500 dark:text-gray-400 ml-[42px]">
                            {rubroConfig.newProductsSubtitle}
                        </p>
                    </div>
                    <Button variant="outline" asChild className="border-secondary/30 text-secondary dark:text-secondary/80 hover:bg-secondary/10 hover:border-secondary/60 transition-all duration-300">
