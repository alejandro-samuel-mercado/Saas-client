...
export function Marquee() {
    ...
    if (isLoading) {
        return (
            <div className="w-screen relative left-1/2 -translate-x-1/2 h-16 bg-gradient-to-r from-secondary via-secondary to-secondary animate-pulse" />
        );
    }

    const getMarqueeItems = () => {
        if (!config?.marqueeText) return rubroConfig.marqueeItems;
        if (Array.isArray(config.marqueeText)) {
            return config.marqueeText.length > 0
                ? config.marqueeText
                : rubroConfig.marqueeItems;
        }
        const legacyText = config.marqueeText as any;
        if (typeof legacyText === "string" && legacyText.trim()) {
            return [legacyText];
        }
        return rubroConfig.marqueeItems;
    };
    ...
    return (
        <div className="w-screen relative left-1/2 -translate-x-1/2 overflow-hidden bg-gradient-to-r from-secondary to-primary pointer-events-none">
